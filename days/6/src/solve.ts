...
const parseInput = (lines: string[]): Race[] => {
  const times = lines[0].split(/\s+/).slice(1).map(Number);
  const targets = lines[1].split(/\s+/).slice(1).map(Number);

  return times.map((time, idx) => ({ time, target: targets[idx] }));
};
...
export const partA: Solver = (lines: string[]) => {
  ...
  for (const race of races) {
    ...
    for (let hold = 1; hold < race.time; hold++) {
      const dist = (race.time - hold) * hold;
      if (dist > race.target) {
        numWins++;
      }
    }
...
const parseInputFixed = (lines: string[]): Race => ({
  time: parseInt(lines[0].split(":")[1].replace(/\s+/g, "")),
  target: parseInt(lines[1].split(":")[1].replace(/\s+/g, "")),
});

export const partB: Solver = (lines: string[]) => {
  const race = parseInputFixed(lines);
  let numWins = 0;
  for (let hold = 1; hold < race.time; hold++) {
    const dist = (race.time - hold) * hold;
    // A = race.time, B = race.target
    // d(x) = x^2 - Ax - B, find x intercepts
    if (dist > race.target) {
      numWins++;
    }
  }
