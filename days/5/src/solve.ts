...
const parseRules = (rules: string[]) =>
  rules.map((s) => {
    const [before, after] = s.split("|").map((n) => Number.parseInt(n));
    return { before, after };
  });

const parseUpdates = (updates: string[]) =>
  updates.map((u) => u.split(",").map((n) => Number.parseInt(n)));

const parseInput = (lines: string[]): Input => ({
  rules: parseRules(lines.slice(0, lines.indexOf(""))),
  updates: parseUpdates(lines.slice(lines.indexOf("") + 1)),
});
...
export const partA: Solver = (lines: string[]) => {
  ...
  const correctUpdates = updates.filter((_, i) =>
    rules.every((rule) => {
