...
const parseRobots = (lines: string[]): Robot[] =>
  lines.map((l) => {
    const [position, velocity] = l
      .replace("p=", "")
      .split(" v=")
      .map((s) => s.split(",").map((n) => Number.parseInt(n)) as Tuple<number>);
...
const positionAtTime = (
  ...
  modulo(r.position[0] + r.velocity[0] * t, w),
  modulo(r.position[1] + r.velocity[1] * t, h),
];
...
export const partA: Solver = (lines: string[]) => {
  ...
  const after = robots
    .map((r) => positionAtTime(r, width, height, 100))
    .filter(([x, y]) => x !== (width - 1) / 2 && y !== (height - 1) / 2);
  const quads = groupByFunction(
    after,
    ([x, y]) => (x * 2 < width ? 1 : 0) + (y * 2 < height ? 2 : 0)
  );
...
export const partB: Solver = async (lines: string[]) => {
  ...
  for (let t = 0; true; t++) {
    const after = robots
      .map((r) => positionAtTime(r, width, height, t))
      .reduce((acc, [x, y]) => {
        acc.update(x, y, (n) => n + 1);
        return acc;
      }, new SparseGrid(0, (v) => (v > 0 ? `${v}` : ".")));

    if (after.size() === robots.length) {
      console.log(after.toString(0));
      return t;
    }
  }
};
