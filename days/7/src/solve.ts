...
type Equation = { test: number; numbers: number[] };

const evaluate = (eq: Equation): boolean => {
  const q = eq.numbers.slice(1);
  const f = (n: number) => n <= eq.test;
  let perms: number[] = [eq.numbers[0]].filter(f);

  while (q.length && perms.length) {
    // biome-ignore lint/style/noNonNullAssertion: <explanation>
    const next = q.shift()!;
    const lastPerms = perms;
    perms = [];

    for (const p of lastPerms) {
      perms.push(p + next, p * next);
    }

    perms = perms.filter(f);
  }

  if (q.length) return false;
  return perms.some((p) => p === eq.test);
};

const evaluate2 = (eq: Equation): boolean => {
  const q = eq.numbers.slice(1);
  const f = (n: number) => n <= eq.test;
  let perms: number[] = [eq.numbers[0]].filter(f);
  ...
  while (q.length && perms.length) {
    ...
    for (const p of lastPerms) {
      perms.push(p + next, p * next, Number.parseInt(`${p}${next}`));
    }
...
export const partA: Solver = (lines: string[]) => {
  const equations = lines.map(parseLine);
  const valid = equations.filter(evaluate);
  return valid.map((eq) => eq.test).reduce(sum);
};

//
export const partB: Solver = (lines: string[]) => {
  const equations = lines.map(parseLine);
  const valid = equations.filter(evaluate2);
  return valid.map((eq) => eq.test).reduce(sum);
};
