...
export class ProgressCache<K extends string, V, R> {
  ...
  private _cache: Record<K, CacheEntry<V, R>> = {} as Record<
    K,
    CacheEntry<V, R>
  >;
  ...
  constructor(filename: string, flushOnChange = false, autoRead = true) {
    ...
    if (autoRead) {
      this.readCache().catch((err) => {
        console.error(
          `Failed to read progress cache (${this._filename}) from disk!`,
          err
        );
      });
    }
  ...
  get incompleteValues(): V[] {
    return Object.values<CacheEntry<V, R>>(this._cache)
      .filter(({ complete }) => !complete)
      .map(({ value }) => value);
  }

  get isComplete(): boolean {
    return !Object.values<CacheEntry<V, R>>(this._cache).some(
      ({ complete }) => !complete
    );
  }
  ...
  flush = async () => {
    try {
      ...
      console.error(
        `Failed to flush progress cache (${this._filename}) to disk!`,
        err
      );
    }
  };
